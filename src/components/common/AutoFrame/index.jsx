...
class AutoFrame extends Component {
  ...
  componentDidMount() {
    const isKbInput = window.document.getElementById('J_isFromKbServ');
  ...
  render() {
    ...
    return (
      <div {...this.props}>
        {target && !errorInfo ? <iframe className="kb-index-frame" src={target} ref={(node) => { this.frameRef = node; return null; }} style={{ height: frameHeight }} onError={this.onError} /> : null}
      </div>
    );
  }
}
