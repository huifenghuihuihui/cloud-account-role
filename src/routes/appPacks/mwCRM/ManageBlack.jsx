...
function ManageBlack({ cloudState }) {
  const { urlManageBlack } = cloudState.mwCRM;
  const { MWTenantSelected, authType, authCode } = cloudState.account;
  const iframeProps = {
    urlPage: urlManageBlack,
    MWTenantSelected,
    authType,
    authCode,
    iframeHight: 1100,
...
ManageBlack.propTypes = {
  cloudState: PropTypes.object,
};
export default connect(mapStateToProps)(ManageBlack);
