...
function ManageManual({ cloudState }) {
  const { urlManageManual } = cloudState.mwCRM;
  const { MWTenantSelected, authType, authCode } = cloudState.account;
  const iframeProps = {
    urlPage: urlManageManual,
    MWTenantSelected,
    authType,
    authCode,
    iframeHight: 1100,
...
ManageManual.propTypes = {
  cloudState: PropTypes.object,
};
export default connect(mapStateToProps)(ManageManual);
