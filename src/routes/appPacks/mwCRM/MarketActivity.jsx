...
function MarketActivity({ cloudState }) {
  const { urlMarketActivity } = cloudState.mwCRM;
  const { MWTenantSelected, authType, authCode } = cloudState.account;
  const iframeProps = {
    urlPage: urlMarketActivity,
    MWTenantSelected,
    authType,
    authCode,
    iframeHight: 1100,
...
export default connect(mapStateToProps)(MarketActivity);
