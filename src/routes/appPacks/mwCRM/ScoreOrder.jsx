...
function ScoreOrder({ cloudState }) {
  const { urlScoreOrder } = cloudState.mwCRM;
  const { MWTenantSelected, authType, authCode } = cloudState.account;
  const iframeProps = {
    urlPage: urlScoreOrder,
    MWTenantSelected,
    authType,
    authCode,
    iframeHight: 1100,
...
ScoreOrder.propTypes = {
  cloudState: PropTypes.object,
};
export default connect(mapStateToProps)(ScoreOrder);
