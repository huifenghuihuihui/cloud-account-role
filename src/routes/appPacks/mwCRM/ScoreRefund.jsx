...
function ScoreRefund({ cloudState }) {
  const { urlScoreRefund } = cloudState.mwCRM;
  const { MWTenantSelected, authType, authCode } = cloudState.account;
  const iframeProps = {
    urlPage: urlScoreRefund,
    MWTenantSelected,
    authType,
    authCode,
    iframeHight: 1100,
...
ScoreRefund.propTypes = {
  cloudState: PropTypes.object,
};
export default connect(mapStateToProps)(ScoreRefund);
