...
function WxTemplate({ cloudState }) {
  const { urlWxTemplate } = cloudState.mwCRM;
  const { MWTenantSelected, authType, authCode } = cloudState.account;
  const iframeProps = {
    urlPage: urlWxTemplate,
    MWTenantSelected,
    authType,
    authCode,
    iframeHight: 1100,
...
WxTemplate.propTypes = {
  cloudState: PropTypes.object,
};
export default connect(mapStateToProps)(WxTemplate);
